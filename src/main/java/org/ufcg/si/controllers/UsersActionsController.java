...
/**
 * This controller class uses JSON data format to be the endpoint of requests
 * related to user actions on the server-side.
 * 
...
public class UsersActionsController {
	 ...
	 *             if can't create the folder
	 */
	@RequestMapping(value = "/newfolder", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	public ResponseEntity<User> addFile(@RequestBody AddNDeleteFileBean body) throws ServletException {
		MeasuringDevice mdADD = new MeasuringDevice("src/main/java/org/ufcg/si/util/monitoring/logging/Log1");
		...
		try {
			...
			User updatedUser = userService.update(dbUser);
			mdADD.endMeasurement();
			mdADD.logMeasurement("ADD");
			return new ResponseEntity<>(updatedUser, HttpStatus.OK);
		} catch (GreenboxException gbe) {
			gbe.printStackTrace();
	 ...
	 * @return the http status and user updated.
	 * @throws Exception
	 *             if can't edit the file
	 */
	@RequestMapping(value = "/editfile", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<User> editFile(@RequestBody EditFileBean requestBody) throws Exception {
		try {
			...
			if (!requestBody.getFileName().equals(requestBody.getClickedFile().getName())) {
				dbUser.editFileName(requestBody.getFileName(), requestBody.getClickedFile().getName(),
						requestBody.getClickedFile().getExtension(), requestBody.getFilePath());
			}

			if (!requestBody.getFileExtension().equals(requestBody.getClickedFile().getExtension())) {
				dbUser.editFileExtension(requestBody.getFileExtension(), requestBody.getFileName(),
						requestBody.getClickedFile().getExtension(), requestBody.getFilePath());
			}

			if (!requestBody.getFileContent().equals(requestBody.getClickedFile().getContent())) {
				dbUser.editFileContent(requestBody.getFileName(), requestBody.getFileContent(),
						requestBody.getFileExtension(), requestBody.getFilePath());
			}
	 ...
	 * @return the http status and user updated.
	 * @throws Exception
	 *             if can't rename the folder
	 */
	@RequestMapping(value = "/renamefolder", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	 ...
	 * @return the http status and user updated.
	 * @throws Exception
	 *             if can't rename the file
	 */
	@RequestMapping(value = "/renamefile", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	 ...
	 * @return the http status and user updated.
	 * @throws Exception
	 *             if can't share the file
	 */
	@RequestMapping(value = "/sharefile", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<User> shareFile(@RequestBody ShareFileBean body) throws Exception {
		try {
			...
			sendingUser.shareFile(receivingUser, body.getName(), body.getFolderPath(), body.getFileExtension(),
					FilePermissions.valueOfIgnoreCase(body.getPermissionLevel()));
	 ...
	 * @return the http status and user updated.
	 * @throws Exception
	 *             if can't show the notifications
	 */
	@RequestMapping(value = "/visit_notifications", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	public ResponseEntity<User> deleteFile(@RequestBody AddNDeleteFileBean requestBody) throws Exception {
		MeasuringDevice mdDEL = new MeasuringDevice("src/main/java/org/ufcg/si/util/monitoring/logging/Log2");
		...
		try {
			...
			long used = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();
			updateUser.setMemoryUsage(used);
			mdDEL.endMeasurement();
			mdDEL.logMeasurement("DEL");
	 ...
	 * @return the http status and user updated.
	 * @throws Exception
	 *             if can't delete the folder
	 */

	@RequestMapping(value = "/deletefolder", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
